...
Emits data for the Sidebar, and for the Pax Extended Sidebar if one is
...
as a tar Sidebar starting at the specified offset and continuing for
512 bytes, or a data object of keys and values to set on the header
object, and eventually encode as a tar Sidebar.
...
Returns `this.needPax` to indicate whether a Pax Extended Sidebar is
...
the same semantics as the tar.Sidebar field of the same name.
